...
11. Split row distribution: context renders both sides; `-` left only; `+` right only. File / hunk headers no longer split — see #11a.
11a. Header collapse + sticky header (render-only; the raw patch buffer is UNCHANGED, so yank / search / anchoring / line numbers stay buffer-driven — only the displayed rows and `+++` navigation change). Routed in `diffView`'s per-line switch ahead of the split / unified renderers:
    - File header (`--- a/X` / `+++ b/X`): collapsed to ONE full-width separator bar via `renderFileBar` (`fileHeaderLabel` + `fileBarBody`): `─── <label>  [X] ───…`. `[X]` = `M`/`A`/`D`/`R` derived from the `---`/`+++` pair (`/dev/null` → A/D, differing paths → R `old → new`, else M). The `---` row hosts the bar (navigable, cursor target); the paired `+++` row renders NOTHING (`continue` in the loop) and is non-navigable — `lineExistsOnSide("+++…", _) == false` so j/k auto-skip never parks on it, and `displayRowsForLine("+++…") == 0` so the wrap-aware reverse map (mouse / scroll) stays consistent. Consequence for tests: a j-walk crossing the `+++` row reaches a given buffer index in one FEWER press than the raw index.
    - Hunk header (`@@ -a,b +c,d @@ sig`): collapsed to a thin full-width rule via `renderHunkRule`; `displayRowsForLine("@@…") == 1`. The numeric range is dropped (it duplicates the gutter line numbers); the `sig` moves to the sticky header.
    - Sticky header: `diffStickyHeader` pins one row at the TOP of the Diff viewport — `▌<file>  ·  <enclosing hunk sig>`. `<file>` follows the top-visible buffer line: `patchInfo.filePaths[top]` in the All view (so the user always knows which file they are reading while scrolling the concat diff), else `SelectedFile`. `<sig>` = `nearestHunkSignature(lines, top)` (backward scan to the enclosing `@@`, stops at a `---` file boundary). Consumes one content row: `diffStickyRows()` returns 1 when `paneHeightDiff > 3`, and `diffViewportHeight()` subtracts it (single source of truth for both the render fill loop and the scroll math). Mouse: `mouseClickDiff` subtracts `diffStickyRows()` from the content row and rejects a click on the sticky row. Pinned by `internal/tui/pane_diff_header_test.go`.
...
14d. Context expand (synthetic `···` rows). Implemented in `internal/diff/expand.go` + `internal/tui/expand.go`; renderer slot in `pane_diff.go::renderSynthBufferLine`.
   - Hidden regions surface as a buffer row reading `··· N lines folded · Enter to expand`. Three gap kinds: `GapKindBOF` (above first hunk), `GapKindMid` (between consecutive hunks, indexed 0..N-2), `GapKindEOF` (below last hunk). EOF requires file lines (file length unknown otherwise). Render shape per mode (`pane_diff.go::renderSynthBufferLine`):
     - Unified: one full-width row, `> ` at col 0 follows cursor / visual membership.
     - Split: mirror the standard split geometry (#8) — same `··· …` body painted on BOTH halves, `│` divider between them, per-side line-number / marker columns blank. `> ` follows `DiffCursor.Side` (Lcursor / Rcursor), matching the rule used by regular split rows; a single-cell body would strand the cursor on the left column and read as "cursor jumped" when Side=RIGHT. The body label degrades through 5 tiers (`pane_diff.go::synthLabel`): `··· N lines folded · Enter to expand` → `··· N lines folded (Enter)` → `··· N lines folded` → `··· N folded` → `···`, so the folded-count signal survives even at the split-engage threshold (halfW=8).
   - Synthetic-row emission: `diff.Expand` emits BOF / Mid `···` rows whenever `ExpandInputs.Placeholders` is true OR `FileLines != nil`. Production (`patchInfo()` in `app.go`) passes `Placeholders: true`, so the BOF / Mid markers are drawn from the hunk-header arithmetic on the very first frame — no file fetch required. This is the anti-jitter contract: the markers are present immediately, so a later `FileContents` prefetch only fills in revealed context and the EOF row, never shifting the BOF / Mid rows the user is already looking at. EOF still requires `FileLines` (the file length is unknowable from the patch alone) and surfaces once the prefetch lands, appended below all hunk bodies so it does not shift on-screen content at the top of a freshly-selected file. Direct `diff.Expand` callers that omit `Placeholders` AND `FileLines` still get the raw patch unchanged (unit-test fixtures rely on this).
...
19a. Cursor index 0 is the synthetic "[*] All (N files)" row, symmetric to the Commits pane's "[*] All commits" row (#15a). Cursor space `[0, len(PR.Files)]`: idx 0 → `selectAllFiles()` (sets `SelectedFile = model.AllFilesPath`); idx 1..N → `selectFile(PR.Files[i-1].Path)`. Loader lands on cursor 0 — the [*] row — and sets `SelectedFile = AllFilesPath` so the splash hands off to a PR-wide overview (concat diff across every file); users drill into a single file via `j` + `Enter` (or `Shift+J` to advance without losing focus). FilesCursor and SelectedFile are kept in sync at boot so the cursor never sits on [*] while the Diff/Commits/Comments columns reflect a single file. Under AllFilesPath:
   ...
   - Visual entry on Files idx 0 is forbidden (`handleKey "v"` returns early with `state.Notice = "visual unavailable on the All row"`); Files yank skips idx 0 (symmetric to the Commits-pane skip for All commits).
...
19b. The Files pane is TREE-ONLY. The flat full-path list and the `t` toggle were retired (`FilesTreeMode` removed from `model.AppState`; `handleKeyFilesFlat` / `autoSelectFlat` / `allFilesRow` / `remapCursorOnTreeToggle` deleted). Every Files row is a `model.FilesRow` (All / dir / file), so `FilesCursor` indexes `filesTreeRows()` directly — there is no `cursor == files index + 1` mapping. All consumers (`handleKeyFiles`, `filesView`, `mouseClickFiles` / `mouseWheelFiles`, `collectFileMatches`, `applySearchCursor`, `yankString`, the Files-modal-Enter branch in `keys.go`) operate on tree rows unconditionally.
20. Tree rendering: dirs render `v <name>/` (expanded) or `> <name>/` (folded); files show basename + bracketed status `[A]/[M]/[D]/[R]` + comment count. Tree row 0 is the FilesRowAll entry — rendered as `[*] All (N files)` (bold; the `[*]` slot mirrors the Commits-pane All row). `buildFilesTree` sorts directories alphabetically then root files, so the first FILE row is the alphabetically-first path (e.g. `docs/api.md` precedes `src/greeting.go`); `advanceFile` (Shift+J/K) walks file rows in that order.
...
22b. Enter on a file row — commit gesture: `selectFile(path)` + `FocusedPane = PaneDiff`. Enter on the All row (FilesRowAll) — `selectAllFiles()` + `FocusedPane = PaneDiff`. Tree dir rows fold / unfold, focus stays. Files zoom modal Enter = same commit gestures (All / file / dir); modal-Enter path lives in `keys.go::handleKey` because it must clear `Modal` first (dir rows `break` out of the Modal.Pane switch so the per-pane handler folds and the modal stays open).
...
24. `j/k` in Comments are row-based scroll, not comment-unit jump — they nudge `AppState.CommentsTop` by ±1 display row via `scrollCommentsBy`, clamped to `[0, commentsMaxTopFor(rows, height)]`. The "current comment" (`AppState.CommentsCursor`, the target of Enter/`r`/Space/visual yank and the source of `syncDiffToCursorComment`) is DERIVED from `CommentsTop` by `commentsCursorFromTop` = the comment whose header is most recent at or before the viewport-top row; inter-thread blank separators belong visually to the preceding thread, so they map to that thread's index. Long bodies are now readable mid-scroll — the previous comment-unit jump skipped over them. The `> ` cursor glyph still paints on a comment's header row, but it appears only when that header is inside the viewport; once the user scrolls into the body the header rolls off-screen and the glyph is invisible by design. `gg` resets both `CommentsTop = 0` and `CommentsCursor = 0`; `G` calls `scrollCommentsToBottom` (CommentsTop = maxTop, cursor re-derived). Diff auto-scroll fires only when the derived cursor crosses a comment boundary — pure intra-comment scrolling does not re-aim the Diff viewport. `h/l` and `backspace` are unbound. `CommentsTop = 0` resets explicitly at every `CommentsCursor = 0` site (Diff Enter handoff `focusCommentsAtCursor`, file selection `selectFile` / `selectAllFiles`, commit selection `autoSelectCommit`, Files-modal-Enter, Commits-modal-Enter, Comments `gg`). Mouse wheel routes through `scrollCommentsBy(dir)`; click parks `CommentsTop` on the clicked comment's header row (clamped to maxTop) so the clicked comment becomes the new top-of-view. `commentIndexAtDisplayRow` adds `CommentsTop` to the pane-relative row before walking the layout so clicks on a scrolled column still resolve. Shared layout helper: `commentsLayout()` returns `(rows, headerAt)` and is consumed by `commentsView` (slice from `CommentsTop`), `scrollCommentsBy` / `scrollCommentsToBottom` / `commentsCursorFromTop`, and indirectly by the hit-test walker; keeping a single source of truth prevents the scroll math and the render from drifting. Viewport height fallback ladder mirrors `diffViewportHeight()`: `paneHeightComments → height-16 → 5`.
