...
14b. `gg` is a true two-key sequence. First `g` records `AppState.DiffPendingPrefix = "g"`; next `g` runs gotoTop. Any non-`g` key clears pending AND falls through to its normal dispatch (`g` then `k` moves up by one). Pending cleared by every keystroke that exits Diff key context (`tab`, `shift+tab`, `J`, `K`, `v`, `?`, `esc`, `y`). Forward-compatible with future `g`-prefix maps. Locked by F4d / F4e / F4f in `e2e/tests/05_pane_diff.test.mjs`.
