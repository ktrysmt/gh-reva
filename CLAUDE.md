...
30f. Per-pane click semantics: `handleMouseClick` always sets `FocusedPane = hit.Pane`, then dispatches when `OnTitle == false`. Files: `mouseClickFiles` moves `FilesCursor` AND calls `selectFile(path)` so Diff follows the click — j/k stays cursor-only (#19) because per-keystroke Diff rebuild is sluggish under repeat, but a click is a deliberate one-shot gesture and the user expects the Diff column to update immediately. Tree-mode dir rows fold / unfold instead of committing. Focus stays on Files (the caller already set it) so the user can keep clicking through files without re-targeting. Commits: `mouseClickCommits` sets `CommitsCursor` and `autoSelectCommit` (mirrors j/k #16, including the synthetic "All commits" row at index 0). Diff: `mouseClickDiff` resolves the buffer line via `bufferLineAtDiffDisplayRow` (wrap-aware reverse of `displayRowsForLine`, so multi-row wraps land on the correct buffer index) and calls `scrollDiffIntoView`. Comments: `mouseClickComments` resolves the flat-comment index via `commentIndexAtDisplayRow` (walks the same shape `commentsView` builds — header + body wraps interleaved with blank separator rows return -1) and calls `syncDiffToCursorComment`.
...
30i. Layout measurement: `(*Model).measureLayout` populates `paneWidth* / paneHeight*` from the current terminal size. Called by `View()` before pane renderers run, and by `handleMouse` before hit-testing — Bubbletea's value-receiver `Update` delivers a fresh Model each tick that does not carry View's in-flight measurements. Pre-WindowSize (m.width<=0) and stacked fallback (bodyHeight<8) skip the assignment so existing zero-width fallbacks downstream stay unchanged.

Locked by `internal/tui/mouse_test.go::TestPaneAt_*` / `TestMouse_*` and `e2e/tests/22_mouse.test.mjs::M1..M6`.
