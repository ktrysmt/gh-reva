...
38. **Diff cells use bg-for-change + per-token syntax fg** (`internal/tui/syntax.go::styledDiffCell`). `+` rows get `DiffPlusBg` row-wide AND chroma syntax-highlighted fg per token; `-` rows likewise with `DiffMinusBg`. Context rows stay flat-fg (`DiffContext`) — they outnumber changed rows by ~3:1 and tokenizing them blew the e2e render budget. File / hunk headers stay flat-fg too. The cell's leading marker (`+`/`-`/space) is excluded from the chroma lexer (parses as a syntax error in most languages) and re-emitted under the same bg.
