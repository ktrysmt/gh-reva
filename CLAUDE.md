...
### Files pane
19. `j/k` in Files auto-selects → `selectFile(path)`. Visual mode gates this. `selectFile` resets `DiffCursor`, `DiffViewport.Top`, `CommitsCursor`, `CommentsCursor` only when path changes.
20. Tree mode (`t` toggles): dirs render `v <name>/` (expanded) or `> <name>/` (folded); files show basename + status + comment count.
21. `autoSelectTree` skips `selectFile` on dir rows so folding doesn't clobber Diff.
22. `remapCursorOnTreeToggle` preserves the conceptual cursor position when toggling flat ⇄ tree.
22b. Enter binds only to dir fold/unfold in tree mode. Enter on a file row (flat or tree) is a no-op.

### Comments pane
23. Diff-cursor coupling: `commentsView` shows ONLY threads anchored at the Diff cursor's current buffer line (the `◆` rows). When cursor is not on a `◆` row, column reads `(no comment at cursor)`. Visible-thread set computed by `threadsForCursor` (maps `DiffCursor.Line` through `patchNewLineNumbers`). `flatComments` is scoped to `threadsForCursor` so cursor index never drifts past visible content.
...
29. Enter is the comment-input / focus-handoff gesture; never quits or moves focus across columns. Backspace unbound everywhere. Bindings:
    - Files (tree, dir): fold/unfold.
    - Files (file row, flat or tree): no-op (j/k drives sync).
    - Files / Commits zoom modal: close modal AND shift FocusedPane to PaneDiff (CommentsCursor reset). Tree-mode dir rows still fold/unfold.
    - Commits (normal pane): no-op.
    - Diff (uncommented row): inline compose at cursor (or visual range — #14, #27d). Header / hunk rows no-op.
    - Diff (row with comments): hand off to Comments zoom modal.
...
42b. Modal focus restore: `ModalState.Origin` records the opener pane. `toggleModal` reads `m.state.FocusedPane` at open time; `openCommentsModalAtCursor` (Diff Enter handoff) hardcodes `Origin = PaneDiff`. Close gestures (`<space>`, `q`, `Esc`, `Ctrl+C`) route through `(*Model).closeModal`, which restores `FocusedPane = Origin` before clearing `Modal`. `Tab` / `Shift+Tab` also call `closeModal` first so next/prev reads the user's pre-modal pane (otherwise the modal-side `FocusedPane` — e.g. Comments after a Diff Enter handoff — would steer focus to a surprising destination). `?` (Help) likewise calls `closeModal` so post-Help close lands on the opener. Files-modal-Enter / Commits-modal-Enter intentionally bypass `closeModal` and explicitly set `FocusedPane = PaneDiff` (the user picked a row to inspect). Locked by `compose_test.go::TestSpaceClose_*` / `TestQClose_RestoresFocusToOrigin` / `TestEscClose_*` and `e2e/tests/13_modal.test.mjs::F-modal-9/10`.
