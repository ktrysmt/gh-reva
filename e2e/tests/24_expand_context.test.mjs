...
test('Expand-1: synthetic `···` row appears for BOF/Mid/EOF after prefetch', async () => {
  ...
  assert.match(diff, /4 lines folded/, `BOF synthetic expected:\n${diff}`)
  assert.match(diff, /12 lines folded/, `Mid synthetic expected:\n${diff}`)
  assert.match(diff, /7 lines folded/, `EOF synthetic expected:\n${diff}`)
  await quit(s)
})
...
test('Expand-2: Enter on BOF synthetic reveals 4 lines (capped) and removes the synthetic', async () => {
  ...
  //   idx 0: --- a/file.txt   (collapsed file bar — navigable)
  //   idx 1: +++ b/file.txt   (folded into the bar — non-navigable, skipped)
  //   idx 2: SyntheticLine (BOF, 4 hidden)
  // The `+++` row is skipped by j/k auto-skip, so one j lands on the BOF
  // synthetic.
  ...
  // appear as context.
  const diff = paneText(await s.text(), 'Diff')
  assert.doesNotMatch(diff, /4 lines folded/, `BOF synthetic should be gone:\n${diff}`)
